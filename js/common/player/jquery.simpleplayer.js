...
(function($) {
    $.fn.player = function(settings) {
        ...
        this.each(function() {
            $(this).before('<div class="simple-player-container" style="background-color: #ddd; padding: 0 10px 5px 5px; display: none">');
            $(this).after('</div>');
            $(this).parent().find('.simple-player-container').prepend(
                '<div><ul>' +
                    '<li style="display: inline-block; padding: 0 5px; float: left;"><a style="text-decoration: none;"' +
                    ' class="start-button" href="javascript:void(0)">' + playControl + '</a></li>' +
                     '<li class="a_close">X</li>'+
...
$(document).on('click','.a_close',function () {
    $(this).parents('.simple-player-container').fadeOut();
})
